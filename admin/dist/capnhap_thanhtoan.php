...
if ($row['trangthai'] !== 'dang_giao') {
    ...
    echo "<script>alert('Không thể thanh toán đơn này. Trạng thái hiện tại: $msg'); window.location='index.php?page_layout=danhsachdonhang';</script>";
    exit;
}

// ---------- Cập nhật trạng thái ----------
$stmt = $ketnoi->prepare("UPDATE donhang SET trangthai='hoan_thanh' WHERE iddonhang=?");
$stmt->bind_param("i", $iddonhang);
$stmt->execute();
$stmt->close();
$ketnoi->close();
